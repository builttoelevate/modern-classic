...
// invalidated by, link-relationship changes. Either signal alone
// is fragile — link writes can fail silently (logged but swallowed
// in the booking endpoint), and a parent might unlink someone
// after a shared booking is on the calendar. The manifest is the
// always-correct breadcrumb back to every member of a specific
// group, regardless of what happens to the relationship graph
// downstream.
//
// Currently the manifest is populated at booking creation only.
// /my-bookings still uses the phone-based self-heal in
// groupSelfHeal.ts as its recovery path; the manifest sits as the
// system of record so a future read path can do a single Redis GET
// instead of a phone scan.
